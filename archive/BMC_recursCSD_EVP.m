...
% Figure out ascending or descending. Can I get that from NS.Header? How
% can I import and use that information for the channels. 
...
for i = 1:size(varName,2) %big loop
    ...
    elseif strcmp(string(SessionParam.SortDirection(idxSessionParam)),'descending')
       ...
       sinkBtm = electrodeLength - SessionParam.EvalSink(idxSessionParam) + 1; %The plus 1 is needed to count inclusively
    end
    ...
    figure(1),cla
        for chan = 1:size(CSD,1)
           ...
           vline(0);hline(0);
           if chan ==1
                  title(strcat(string(SessionParam.Datestr(idxSessionParam)),' EVP CSD Line sinkBtm =',string(sinkBtm)));
           end
        end
    ...
    title(strcat(string(SessionParam.Datestr(idxSessionParam)),' EVP CSD filt sinkBtm =',string(sinkBtm)));
