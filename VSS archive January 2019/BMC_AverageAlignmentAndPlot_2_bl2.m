...
clearvars -except AlignedCSD SessionParams SAVEALLCSD Unq_cond varName
flag_subtractbasline = true;
sortdirection = 'descending';
text = 'Non Pref stim double baseline subtract';
...
%% Sub baseline again
    if flag_subtractbasline
        bl =1:50;
        for b = 1:size(cutACE,1)
            bl_cutACE = mean(cutACE(b,bl),2);
            blcutACE(b,:) = (cutACE(b,:)-bl_cutACE); 
        end
    end
cutACE = blcutACE;
